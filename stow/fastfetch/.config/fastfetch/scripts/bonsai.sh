...
if ! [ "$lifeStart" -eq "$lifeStart" ] 2>/dev/null; then
	printf '%s\n' "Error: --life ($lifeStart) invalid: must be an integer" 1>&2; exit 1

elif ! [ "$multiplier" -eq "$multiplier" ] 2>/dev/null; then
	printf '%s\n' "Error: --multiplier ($multiplier) invalid: must be an integer" 1>&2; exit 1

elif ! [ "$baseType" -eq "$baseType" ] 2>/dev/null; then
	printf '%s\n' "Error: --base ($baseType) invalid: must be an integer" 1>&2; exit 1

# ensure ranges
elif [ "$baseType" -lt 0 ]; then
	printf '%s\n' "Error: --base ($baseType) invalid: out of range" 1>&2; exit 1

elif [ "$lifeStart" -lt 1 ] || [ "$lifeStart" -gt 200 ]; then
	printf '%s\n' "Error: --life ($lifeStart) invalid: out of range" 1>&2; exit 1

elif [ "$multiplier" -lt 0 ] || [ "$multiplier" -gt 20 ]; then
	printf '%s\n' "Error: --multiplier ($multiplier) invalid: out of range" 1>&2; exit 1

elif [ "$seed" -lt 0 ] || [ "$seed" -gt 32767 ]; then
	printf '%s\n' "Error: --seed ($seed) invalid: out of range" 1>&2; exit 1

# ensure floats are less than 0
elif [ "$(printf '%s\n' "$timeStep < 0" | bc -l)" -eq 1 ]; then
	printf '%s\n' "Error: --timestep ($timeStep) invalid: out of range" 1>&2; exit 1

elif [ "$(printf '%s\n' "$timeWait < 0" | bc -l)" -eq 1 ]; then
	printf '%s\n' "Error: --wait ($timeWait) invalid: out of range" 1>&2; exit 1
fi
...
branch() {
	...
	while [ "$life" -gt 0 ]; do
		...
		if ((live)); then
			...
			elapsed="$(awk 'NR==1{print $2}' "$tmpFile")"
