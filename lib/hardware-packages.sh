...
_create_gpu_conf() {
    ...
    local template_file="$DOTFILES_DIR/stow/hypr/.config/hypr/gpu.conf.template"

    # If no GPU detected and template exists, copy template
    if [[ "$has_nvidia" == "false" && "$has_amd" == "false" && "$has_intel" == "false" ]]; then
        if [[ -f "$template_file" ]]; then
            log_info "No GPU detected, using template..."
