...
_generate_gpu_packages() {
    ...
    for pattern in "${patterns[@]}"; do
        # Search official packages (|| true: no matches is fine — grep's exit 1
        # would fire the installer's ERR trap inside the subshell and leak
        # rollback log output into the package list)
        while IFS= read -r pkg; do
            [[ -n "$pkg" ]] && official_packages+=("$pkg")
        done < <(pacman -Qq | grep -i "$pattern" | grep -v "^lib32-" || true)

        # Search lib32 packages separately
        while IFS= read -r pkg; do
            [[ -n "$pkg" ]] && official_packages+=("$pkg")
        done < <(pacman -Qq | grep -i "^lib32-" | grep -i "$pattern" || true)
    done
    ...
    local official_file="$DOTFILES_DIR/packages/hardware/gpu-${gpu_type}.package"
    {
        ...
        echo "# DO NOT EDIT - regenerated by hardware detection"
        echo ""
        # Filter to valid package names: interleaved log/trap output must not
        # end up in the package file (corrupted gpu-amd.package on 2026-07-03)
        printf '%s\n' "${official_only[@]}" | grep -E '^[a-z0-9@._+-]+$' | sort -u || true
    } > "$official_file"
    ...
    local aur_file="$DOTFILES_DIR/packages/hardware/gpu-${gpu_type}-aur.package"
    {
        ...
        echo "# DO NOT EDIT - regenerated by hardware detection"
        echo ""
        printf '%s\n' "${aur_packages[@]}" | grep -E '^[a-z0-9@._+-]+$' | sort -u || true
    } > "$aur_file"
...
# Internal: Check generated GPU config for Hyprland
